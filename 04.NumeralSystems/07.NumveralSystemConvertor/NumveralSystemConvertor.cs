   ...
   static int ConvertToDecimal(string number, int baseFrom)
    {
        ...
        for (int i = 0; i < number.Length; i++)
        {
            if (number[i] > '9')
            {
                decNumber += (number[i] - '7') * (int)Math.Pow(baseFrom, (number.Length - 1 - i));
            }
            else
            {
                decNumber += (number[i] - '0') * (int)Math.Pow(baseFrom, (number.Length - 1 - i));
    ...
    static void Main()
    {
        ...
        {
            ConvertFromDec(ConvertToDecimal(number, s), d);
        }
    }   
}
